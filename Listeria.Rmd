...
#Reading the data
Listeria = read.csv("~/Desktop/Listeria_monocytogenes/Listeria_monocytogenes_R_input.csv", sep=";")
dim(Listeria) #analising input

#getting TPM values
Listeria_expr =as.data.frame(Listeria[,2:ncol(Listeria)])
dim(Listeria_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Listeria_expr) = Listeria[,1]

#filtering out missing values
gsg = goodSamplesGenes(Listeria_expr,verbose = 3)
if (!gsg$allOK)
{
Listeria_expr=Listeria_expr[gsg$goodSamples,gsg$goodGenes]
}
...
sampleTree = hclust(dist(Listeria_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Listeria_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Listeria_expr, power = 12, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Listeria_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Listeria_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Listeria_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Listeria_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Listeria_expr), cex.main=2)
...
##############################
multi_species_df_Listeria_temporary = as.data.frame(ME)
rownames(multi_species_df_Listeria_temporary) =generic_stress_sample_names
Listeria_expr_1 = scale(Listeria_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Listeria_expr_1)*36)
Module_names <- rep(colnames(Listeria_expr_1),36)
lengthOfVector_col=ncol(Listeria_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Listeria_expr_1[j,i]
Module_names[position_ME] <-      colnames(Listeria_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Listeria_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_Listeria_temporary)=module_renamed
multi_species_df_Listeria=multi_species_df_Listeria_temporary
write.csv(Listeria_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
colnames(multi_species_df_Listeria_temporary)=module_renamed
multi_species_df_Listeria=cbind(multi_species_df_Listeria,multi_species_df_Listeria_temporary)
write.csv(Listeria_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_Listeria
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="Listeria_non_correlated"
colnames(multi_species_df_Listeria_temporary)=module_renamed
multi_species_df_Listeria=cbind(multi_species_df_Listeria,multi_species_df_Listeria_temporary)
write.csv(Listeria_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_Listeria
...
#################################
write.csv(multi_species_df_Listeria,"multi_species_df_Listeria.csv")
#################################
```