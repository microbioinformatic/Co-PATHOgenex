...
#Reading the data
Acinetobacter_baumannii = read.csv("~/Desktop/Acinetobacter_baumannii/Acinetobacter_baumannii_R_input.csv", sep=";")
...
gsg = goodSamplesGenes(Acinetobacter_baumannii_expr,verbose = 3)
if (!gsg$allOK)
{
Acinetobacter_baumannii_expr=Acinetobacter_baumannii_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Acinetobacter_baumannii_expr)[2]
```
...
sampleTree = hclust(dist(Acinetobacter_baumannii_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Acinetobacter_baumannii_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Acinetobacter_baumannii_expr, power = 14, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Acinetobacter_baumannii_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Acinetobacter_baumannii_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Acinetobacter_baumannii_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Acinetobacter_baumannii_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Acinetobacter_baumannii_expr), cex.main=2)
...
##############################
multi_species_df_ACIB_temporary = as.data.frame(ME)
rownames(multi_species_df_ACIB_temporary) =generic_stress_sample_names
Acinetobacter_baumannii_expr_1 = scale(Acinetobacter_baumannii_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Acinetobacter_baumannii_expr_1)*36)
Module_names <- rep(colnames(Acinetobacter_baumannii_expr_1),36)
lengthOfVector_col=ncol(Acinetobacter_baumannii_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Acinetobacter_baumannii_expr_1[j,i]
Module_names[position_ME] <-      colnames(Acinetobacter_baumannii_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Acinetobacter_baumannii_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_ACIB_temporary)=module_renamed
multi_species_df_ACIB=multi_species_df_ACIB_temporary
write.csv(Acinetobacter_baumannii_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_ACIB_temporary)=module_renamed
multi_species_df_ACIB=cbind(multi_species_df_ACIB,multi_species_df_ACIB_temporary)
write.csv(Acinetobacter_baumannii_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_ACIB
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="ACIB_non_correlated"
colnames(multi_species_df_ACIB_temporary)=module_renamed
multi_species_df_ACIB=cbind(multi_species_df_ACIB,multi_species_df_ACIB_temporary)
write.csv(Acinetobacter_baumannii_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_ACIB
...
#################################
write.csv(multi_species_df_ACIB,"multi_species_df_ACIB.csv")
#################################
```