...
#Reading the data
Salmonella = read.csv("~/Desktop/Salmonella_enterica/Salmonella_enterica_R_input.csv", sep=";")
...
gsg = goodSamplesGenes(Salmonella_expr,verbose = 3)
if (!gsg$allOK)
{
Salmonella_expr=Salmonella_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Salmonella_expr)[2]
```
...
sampleTree = hclust(dist(Salmonella_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Salmonella_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Salmonella_expr, power = 12, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Salmonella_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Salmonella_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Salmonella_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Salmonella_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Salmonella_expr), cex.main=2)
...
##############################
multi_species_df_SALMT_temporary = as.data.frame(ME)
rownames(multi_species_df_SALMT_temporary) =generic_stress_sample_names
Salmonella_expr_1 = scale(Salmonella_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Salmonella_expr_1)*36)
Module_names <- rep(colnames(Salmonella_expr_1),36)
lengthOfVector_col=ncol(Salmonella_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Salmonella_expr_1[j,i]
Module_names[position_ME] <-      colnames(Salmonella_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Salmonella_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_SALMT_temporary)=module_renamed
multi_species_df_SALMT=multi_species_df_SALMT_temporary
write.csv(Salmonella_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_SALMT_temporary)=module_renamed
multi_species_df_SALMT=cbind(multi_species_df_SALMT,multi_species_df_SALMT_temporary)
write.csv(Salmonella_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_SALMT
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="SALMT_non_correlated"
colnames(multi_species_df_SALMT_temporary)=module_renamed
multi_species_df_SALMT=cbind(multi_species_df_SALMT,multi_species_df_SALMT_temporary)
write.csv(Salmonella_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_SALMT
...
#################################
write.csv(multi_species_df_SALMT,"multi_species_df_SALMT.csv")
#################################
```