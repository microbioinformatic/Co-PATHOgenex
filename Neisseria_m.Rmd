...
#Reading the data
Neisseria_m = read.csv("~/Desktop/Neisseria_meningitidis//Neisseria_meningitidis_R_input.csv", sep=";")
dim(Neisseria_m) #analising input

#getting TPM values
Neisseria_m_expr =as.data.frame(Neisseria_m[,2:ncol(Neisseria_m)])
dim(Neisseria_m_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Neisseria_m_expr) = Neisseria_m[,1]

#filtering out missing values
gsg = goodSamplesGenes(Neisseria_m_expr,verbose = 3)
if (!gsg$allOK)
{
Neisseria_m_expr=Neisseria_m_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Neisseria_m_expr)[2]
```
...
sampleTree = hclust(dist(Neisseria_m_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Neisseria_m_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
## Note Power=5, R2 over 0.80, module size=15

## Network generation

```{r Networkgeneration}
net = blockwiseModules(Neisseria_m_expr, power = 5, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Neisseria_m_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Neisseria_m_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Neisseria_m_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Neisseria_m_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Neisseria_m_expr), cex.main=2)
...
##############################
multi_species_df_NMEN_temporary = as.data.frame(ME)
rownames(multi_species_df_NMEN_temporary) =generic_stress_sample_names
Neisseria_m_expr_1 = scale(Neisseria_m_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Neisseria_m_expr_1)*36)
Module_names <- rep(colnames(Neisseria_m_expr_1),36)
lengthOfVector_col=ncol(Neisseria_m_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Neisseria_m_expr_1[j,i]
Module_names[position_ME] <-      colnames(Neisseria_m_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Neisseria_m_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_NMEN_temporary)=module_renamed
multi_species_df_NMEN=multi_species_df_NMEN_temporary
write.csv(Neisseria_m_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_NMEN_temporary)=module_renamed
multi_species_df_NMEN=cbind(multi_species_df_NMEN,multi_species_df_NMEN_temporary)
write.csv(Neisseria_m_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_NMEN
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="NMEN_non_correlated"
colnames(multi_species_df_NMEN_temporary)=module_renamed
multi_species_df_NMEN=cbind(multi_species_df_NMEN,multi_species_df_NMEN_temporary)
write.csv(Neisseria_m_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_NMEN
...
#################################
write.csv(multi_species_df_NMEN,"multi_species_df_NMEN.csv")
#################################
```