...
#Reading the data
Vibrio = read.csv("~/Desktop/Vibrio_cholerae/Vibrio_cholerae_R_input.csv", sep=";")
dim(Vibrio) #analising input

#getting TPM values
Vibrio_expr =as.data.frame(Vibrio[,2:ncol(Vibrio)])
dim(Vibrio_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Vibrio_expr) = Vibrio[,1]

#filtering out missing values
gsg = goodSamplesGenes(Vibrio_expr,verbose = 3)
if (!gsg$allOK)
{
Vibrio_expr=Vibrio_expr[gsg$goodSamples,gsg$goodGenes]
}
...
sampleTree = hclust(dist(Vibrio_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Vibrio_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Vibrio_expr, power = 10, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Vibrio_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Vibrio_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Vibrio_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Vibrio_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Vibrio_expr), cex.main=2)
...
##############################
multi_species_df_Vibrio_temporary = as.data.frame(ME)
rownames(multi_species_df_Vibrio_temporary) =generic_stress_sample_names
Vibrio_expr_1 = scale(Vibrio_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Vibrio_expr_1)*36)
Module_names <- rep(colnames(Vibrio_expr_1),36)
lengthOfVector_col=ncol(Vibrio_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Vibrio_expr_1[j,i]
Module_names[position_ME] <-      colnames(Vibrio_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Vibrio_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_Vibrio_temporary)=module_renamed
multi_species_df_Vibrio=multi_species_df_Vibrio_temporary
write.csv(Vibrio_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_Vibrio_temporary)=module_renamed
multi_species_df_Vibrio=cbind(multi_species_df_Vibrio,multi_species_df_Vibrio_temporary)
write.csv(Vibrio_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_Vibrio
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="Vibrio_non_correlated"
colnames(multi_species_df_Vibrio_temporary)=module_renamed
multi_species_df_Vibrio=cbind(multi_species_df_Vibrio,multi_species_df_Vibrio_temporary)
write.csv(Vibrio_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_Vibrio
...
#################################
write.csv(multi_species_df_Vibrio,"multi_species_df_Vibrio.csv")
#################################
```