...
#Reading the data
Neisseria_g = read.csv("~/Desktop/Neisseria_gonorrhoeae/Neisseria_gonorrhoeae_R_input.csv", sep=";")
dim(Neisseria_g) #analising input

#getting TPM values
Neisseria_g_expr =as.data.frame(Neisseria_g[,2:ncol(Neisseria_g)])
dim(Neisseria_g_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Neisseria_g_expr) = Neisseria_g[,1]

#filtering out missing values
gsg = goodSamplesGenes(Neisseria_g_expr,verbose = 3)
if (!gsg$allOK)
{
Neisseria_g_expr=Neisseria_g_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Neisseria_g_expr)[2]
```
...
sampleTree = hclust(dist(Neisseria_g_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Neisseria_g_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Neisseria_g_expr, power = 12, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Neisseria_g_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Neisseria_g_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Neisseria_g_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Neisseria_g_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Neisseria_g_expr), cex.main=2)
...
##############################
multi_species_df_NGON_temporary = as.data.frame(ME)
rownames(multi_species_df_NGON_temporary) =generic_stress_sample_names
Neisseria_g_expr_1 = scale(Neisseria_g_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Neisseria_g_expr_1)*36)
Module_names <- rep(colnames(Neisseria_g_expr_1),36)
lengthOfVector_col=ncol(Neisseria_g_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Neisseria_g_expr_1[j,i]
Module_names[position_ME] <-      colnames(Neisseria_g_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Neisseria_g_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_NGON_temporary)=module_renamed
multi_species_df_NGON=multi_species_df_NGON_temporary
write.csv(Neisseria_g_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_NGON_temporary)=module_renamed
multi_species_df_NGON=cbind(multi_species_df_NGON,multi_species_df_NGON_temporary)
write.csv(Neisseria_g_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_NGON
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed=paste(module_renamed,"yellow",sep="")
colnames(multi_species_df_NGON_temporary)=module_renamed
multi_species_df_NGON=cbind(multi_species_df_NGON,multi_species_df_NGON_temporary)
write.csv(Neisseria_g_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_NGON
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="NGON_non_correlated"
colnames(multi_species_df_NGON_temporary)=module_renamed
multi_species_df_NGON=cbind(multi_species_df_NGON,multi_species_df_NGON_temporary)
write.csv(Neisseria_g_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_NGON
...
#################################
write.csv(multi_species_df_NGON,"multi_species_df_NGON.csv")
#################################
```