...
#Reading the data
Sta_aureus_MSSA476 = read.csv("~/Desktop/Staphylococcus_aureus_MSSA476/Staphylococcus_aureus_MSSA476_R_input.csv", sep=";")
dim(Sta_aureus_MSSA476) #analising input

#getting TPM values
Sta_aureus_MSSA476_expr =as.data.frame(Sta_aureus_MSSA476[,2:ncol(Sta_aureus_MSSA476)])
dim(Sta_aureus_MSSA476_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Sta_aureus_MSSA476_expr) = Sta_aureus_MSSA476[,1]

#filtering out missing values
gsg = goodSamplesGenes(Sta_aureus_MSSA476_expr,verbose = 3)
if (!gsg$allOK)
{
Sta_aureus_MSSA476_expr=Sta_aureus_MSSA476_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Sta_aureus_MSSA476_expr)[2]
```
...
sampleTree = hclust(dist(Sta_aureus_MSSA476_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Sta_aureus_MSSA476_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Sta_aureus_MSSA476_expr, power = 9, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Sta_aureus_MSSA476_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Sta_aureus_MSSA476_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Sta_aureus_MSSA476_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Sta_aureus_MSSA476_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Sta_aureus_MSSA476_expr), cex.main=2)
...
##############################
multi_species_df_MSSA476_temporary = as.data.frame(ME)
rownames(multi_species_df_MSSA476_temporary) =generic_stress_sample_names
Sta_aureus_MSSA476_expr_1 = scale(Sta_aureus_MSSA476_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Sta_aureus_MSSA476_expr_1)*36)
Module_names <- rep(colnames(Sta_aureus_MSSA476_expr_1),36)
lengthOfVector_col=ncol(Sta_aureus_MSSA476_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Sta_aureus_MSSA476_expr_1[j,i]
Module_names[position_ME] <-      colnames(Sta_aureus_MSSA476_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Sta_aureus_MSSA476_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_MSSA476_temporary)=module_renamed
multi_species_df_MSSA476=multi_species_df_MSSA476_temporary
write.csv(Sta_aureus_MSSA476_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_MSSA476_temporary)=module_renamed
multi_species_df_MSSA476=cbind(multi_species_df_MSSA476,multi_species_df_MSSA476_temporary)
write.csv(Sta_aureus_MSSA476_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_MSSA476
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="MSSA476_non_correlated"
colnames(multi_species_df_MSSA476_temporary)=module_renamed
multi_species_df_MSSA476=cbind(multi_species_df_MSSA476,multi_species_df_MSSA476_temporary)
write.csv(Sta_aureus_MSSA476_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_MSSA476
...
#################################
write.csv(multi_species_df_MSSA476,"multi_species_df_MSSA476.csv")
#################################
```