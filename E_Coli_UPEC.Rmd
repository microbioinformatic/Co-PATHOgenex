...
#Reading the data
Escherichia_coli_UPEC_536 = read.csv("~/Desktop/E_coli_data/Escherichia_coli_UPEC_536_R_input.csv", sep=";")
...
gsg = goodSamplesGenes(Escherichia_coli_UPEC_536_expr,verbose = 3)
if (!gsg$allOK)
{
Escherichia_coli_UPEC_536_expr=Escherichia_coli_UPEC_536_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Escherichia_coli_UPEC_536_expr)[2]
```
...
sampleTree = hclust(dist(Escherichia_coli_UPEC_536_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Escherichia_coli_UPEC_536_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Escherichia_coli_UPEC_536_expr, power = 12, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Escherichia_coli_UPEC_536_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Escherichia_coli_UPEC_536_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Escherichia_coli_UPEC_536_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Escherichia_coli_UPEC_536_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Escherichia_coli_UPEC_536_expr), cex.main=2)
...
##############################
multi_species_df_UPEC_temporary = as.data.frame(ME)
rownames(multi_species_df_UPEC_temporary) =generic_stress_sample_names
Escherichia_coli_UPEC_536_expr_1 = scale(Escherichia_coli_UPEC_536_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Escherichia_coli_UPEC_536_expr_1)*36)
Module_names <- rep(colnames(Escherichia_coli_UPEC_536_expr_1),36)
lengthOfVector_col=ncol(Escherichia_coli_UPEC_536_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Escherichia_coli_UPEC_536_expr_1[j,i]
Module_names[position_ME] <-      colnames(Escherichia_coli_UPEC_536_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Escherichia_coli_UPEC_536_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_UPEC_temporary)=module_renamed
multi_species_df_UPEC=multi_species_df_UPEC_temporary
write.csv(Escherichia_coli_UPEC_536_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_UPEC_temporary)=module_renamed
multi_species_df_UPEC=cbind(multi_species_df_UPEC,multi_species_df_UPEC_temporary)
write.csv(Escherichia_coli_UPEC_536_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_UPEC
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="UPEC_non_correlated"
colnames(multi_species_df_UPEC_temporary)=module_renamed
multi_species_df_UPEC=cbind(multi_species_df_UPEC,multi_species_df_UPEC_temporary)
write.csv(Escherichia_coli_UPEC_536_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_UPEC
...
multi_species_df_UPEC
##############################
write.csv(multi_species_df_UPEC,"multi_species_df_UPEC.csv")
#################################
```
