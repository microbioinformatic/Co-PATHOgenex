...
#Reading the data
Campylobacter = read.csv("~/Desktop/Campylobacter_jejuni/Campylobacter_jejuni_R_input.csv", sep=";")
dim(Campylobacter) #analising input

#getting TPM values
Campylobacter_expr =as.data.frame(Campylobacter[,2:ncol(Campylobacter)])
dim(Campylobacter_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Campylobacter_expr) = Campylobacter[,1]

#filtering out missing values
gsg = goodSamplesGenes(Campylobacter_expr,verbose = 3)
if (!gsg$allOK)
{
Campylobacter_expr=Campylobacter_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Campylobacter_expr)[2]
```
...
sampleTree = hclust(dist(Campylobacter_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Campylobacter_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Campylobacter_expr, power = 12, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Campylobacter_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Campylobacter_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Campylobacter_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Campylobacter_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Campylobacter_expr), cex.main=2)
...
##############################
multi_species_df_Campy_temporary = as.data.frame(ME)
rownames(multi_species_df_Campy_temporary) =generic_stress_sample_names
Campylobacter_expr_1 = scale(Campylobacter_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Campylobacter_expr_1)*36)
Module_names <- rep(colnames(Campylobacter_expr_1),36)
lengthOfVector_col=ncol(Campylobacter_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Campylobacter_expr_1[j,i]
Module_names[position_ME] <-      colnames(Campylobacter_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Campylobacter_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_Campy_temporary)=module_renamed
multi_species_df_Campy=multi_species_df_Campy_temporary
write.csv(Campylobacter_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_Campy_temporary)=module_renamed
multi_species_df_Campy=cbind(multi_species_df_Campy,multi_species_df_Campy_temporary)
write.csv(Campylobacter_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_Campy
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="Campy_non_correlated"
colnames(multi_species_df_Campy_temporary)=module_renamed
multi_species_df_Campy=cbind(multi_species_df_Campy,multi_species_df_Campy_temporary)
write.csv(Campylobacter_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_Campy
...
#################################
write.csv(multi_species_df_Campy,"multi_species_df_Campy.csv")
#################################
```