...
#Reading the data
Achromobacter_xylosoxidans = read.csv("~/Desktop/Achromobacter_xylosoxidans_SOLR/Achromobacter_xylosoxidans_SOLR_R_input.csv", sep=";")
...
gsg = goodSamplesGenes(Achromobacter_xylosoxidans_expr,verbose = 3)
if (!gsg$allOK)
{
Achromobacter_xylosoxidans_expr=Achromobacter_xylosoxidans_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Achromobacter_xylosoxidans_expr)[2]
```
...
sampleTree = hclust(dist(Achromobacter_xylosoxidans_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Achromobacter_xylosoxidans_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
## Note Power=6, R2 over 0.80, module size=15

## Network generation

```{r Networkgeneration}
net = blockwiseModules(Achromobacter_xylosoxidans_expr, power = 6, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Achromobacter_xylosoxidans_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Achromobacter_xylosoxidans_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Achromobacter_xylosoxidans_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Achromobacter_xylosoxidans_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Achromobacter_xylosoxidans_expr), cex.main=2)
...
##############################
multi_species_df_ACHX_temporary = as.data.frame(ME)
rownames(multi_species_df_ACHX_temporary) =generic_stress_sample_names
Achromobacter_xylosoxidans_expr_1 = scale(Achromobacter_xylosoxidans_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Achromobacter_xylosoxidans_expr_1)*36)
Module_names <- rep(colnames(Achromobacter_xylosoxidans_expr_1),36)
lengthOfVector_col=ncol(Achromobacter_xylosoxidans_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Achromobacter_xylosoxidans_expr_1[j,i]
Module_names[position_ME] <-      colnames(Achromobacter_xylosoxidans_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Achromobacter_xylosoxidans_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_ACHX_temporary)=module_renamed
multi_species_df_ACHX=multi_species_df_ACHX_temporary
write.csv(Achromobacter_xylosoxidans_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_ACHX_temporary)=module_renamed
multi_species_df_ACHX=cbind(multi_species_df_ACHX,multi_species_df_ACHX_temporary)
write.csv(Achromobacter_xylosoxidans_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_ACHX
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="ACHX_non_correlated"
colnames(multi_species_df_ACHX_temporary)=module_renamed
multi_species_df_ACHX=cbind(multi_species_df_ACHX,multi_species_df_ACHX_temporary)
write.csv(Achromobacter_xylosoxidans_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_ACHX
...
#################################
write.csv(multi_species_df_ACHX,"multi_species_df_ACHX.csv")
#################################
```