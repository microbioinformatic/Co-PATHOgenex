...
#Reading the data
Klebsiella = read.csv("~/Desktop/Klebsiella_pneumoniae/Klebsiella_pneumoniae_R_input.csv", sep=";")
dim(Klebsiella) #analising input

#getting TPM values
Klebsiella_expr =as.data.frame(Klebsiella[,2:ncol(Klebsiella)])
dim(Klebsiella_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Klebsiella_expr) = Klebsiella[,1]

#filtering out missing values
gsg = goodSamplesGenes(Klebsiella_expr,verbose = 3)
if (!gsg$allOK)
{
Klebsiella_expr=Klebsiella_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Klebsiella_expr)[2]
```
...
sampleTree = hclust(dist(Klebsiella_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Klebsiella_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Klebsiella_expr, power = 12, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Klebsiella_expr", verbose = 3)
```
...
datME=moduleEigengenes(Klebsiella_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Klebsiella_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Klebsiella_expr), cex.main=2)
...
##############################
multi_species_df_KLEBS_temporary = as.data.frame(ME)
rownames(multi_species_df_KLEBS_temporary) =generic_stress_sample_names
Klebsiella_expr_1 = scale(Klebsiella_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Klebsiella_expr_1)*36)
Module_names <- rep(colnames(Klebsiella_expr_1),36)
lengthOfVector_col=ncol(Klebsiella_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Klebsiella_expr_1[j,i]
Module_names[position_ME] <-      colnames(Klebsiella_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Klebsiella_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_KLEBS_temporary)=module_renamed
multi_species_df_KLEBS=multi_species_df_KLEBS_temporary
write.csv(Klebsiella_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_KLEBS_temporary)=module_renamed
multi_species_df_KLEBS=cbind(multi_species_df_KLEBS,multi_species_df_KLEBS_temporary)
write.csv(Klebsiella_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_KLEBS
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="KLEBS_non_correlated"
colnames(multi_species_df_KLEBS_temporary)=module_renamed
multi_species_df_KLEBS=cbind(multi_species_df_KLEBS,multi_species_df_KLEBS_temporary)
write.csv(Klebsiella_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_KLEBS
...
#################################
write.csv(multi_species_df_KLEBS,"multi_species_df_KLEBS.csv")
#################################
```