...
#Reading the data
Strep_agalactiae = read.csv("~/Desktop/Streptococcus_agalactiae/Streptococcus_agalactiae_R_input.csv", sep=";")
dim(Strep_agalactiae) #analising input

#getting TPM values
Strep_agalactiae_expr =as.data.frame(Strep_agalactiae[,2:ncol(Strep_agalactiae)])
dim(Strep_agalactiae_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Strep_agalactiae_expr) = Strep_agalactiae[,1]

#filtering out missing values
gsg = goodSamplesGenes(Strep_agalactiae_expr,verbose = 3)
if (!gsg$allOK)
{
Strep_agalactiae_expr=Strep_agalactiae_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Strep_agalactiae_expr)[2]
```
...
sampleTree = hclust(dist(Strep_agalactiae_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Strep_agalactiae_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Strep_agalactiae_expr, power = 10, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Strep_agalactiae_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Strep_agalactiae_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Strep_agalactiae_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Strep_agalactiae_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Strep_agalactiae_expr), cex.main=2)
...
##############################
multi_species_df_STAGA_temporary = as.data.frame(ME)
rownames(multi_species_df_STAGA_temporary) =generic_stress_sample_names
Strep_agalactiae_expr_1 = scale(Strep_agalactiae_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Strep_agalactiae_expr_1)*36)
Module_names <- rep(colnames(Strep_agalactiae_expr_1),36)
lengthOfVector_col=ncol(Strep_agalactiae_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Strep_agalactiae_expr_1[j,i]
Module_names[position_ME] <-      colnames(Strep_agalactiae_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Strep_agalactiae_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_STAGA_temporary)=module_renamed
multi_species_df_STAGA=multi_species_df_STAGA_temporary
write.csv(Strep_agalactiae_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_STAGA_temporary)=module_renamed
multi_species_df_STAGA=cbind(multi_species_df_STAGA,multi_species_df_STAGA_temporary)
write.csv(Strep_agalactiae_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_STAGA
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="STAGA_non_correlated"
colnames(multi_species_df_STAGA_temporary)=module_renamed
multi_species_df_STAGA=cbind(multi_species_df_STAGA,multi_species_df_STAGA_temporary)
write.csv(Strep_agalactiae_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_STAGA
...
#################################
write.csv(multi_species_df_STAGA,"multi_species_df_STAGA.csv")
#################################
```