...
#Reading the data
Haemophilus = read.csv("~/Desktop/Haemophilus_in/Haemophilus_influenzae_R_input2.csv", sep=";")
dim(Haemophilus) #analising input

#getting TPM values
Haemophilus_expr =as.data.frame(Haemophilus[,2:ncol(Haemophilus)])
dim(Haemophilus_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Haemophilus_expr) = Haemophilus[,1]

#filtering out missing values
gsg = goodSamplesGenes(Haemophilus_expr,verbose = 3)
if (!gsg$allOK)
{
Haemophilus_expr=Haemophilus_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Haemophilus_expr)[2]
```
...
sampleTree = hclust(dist(Haemophilus_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Haemophilus_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Haemophilus_expr, power = 12, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Haemophilus_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Haemophilus_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Haemophilus_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Haemophilus_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Haemophilus_expr), cex.main=2)
...
##############################
multi_species_df_HINF_temporary = as.data.frame(ME)
rownames(multi_species_df_HINF_temporary) =generic_stress_sample_names
Haemophilus_expr_1 = scale(Haemophilus_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Haemophilus_expr_1)*33)
Module_names <- rep(colnames(Haemophilus_expr_1),33)
lengthOfVector_col=ncol(Haemophilus_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:33) {
ME_lengthOfVector[position_ME] <- Haemophilus_expr_1[j,i]
Module_names[position_ME] <-      colnames(Haemophilus_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*33+1
}
stress=rep(generic_stress_sample_names,ncol(Haemophilus_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_HINF_temporary)=module_renamed
multi_species_df_HINF=multi_species_df_HINF_temporary
write.csv(Haemophilus_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
...
module_renamed
colnames(multi_species_df_HINF_temporary)=module_renamed
multi_species_df_HINF=cbind(multi_species_df_HINF,multi_species_df_HINF_temporary)
write.csv(Haemophilus_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_HINF
############################################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="HINF_non_correlated"
colnames(multi_species_df_HINF_temporary)=module_renamed
multi_species_df_HINF=cbind(multi_species_df_HINF,multi_species_df_HINF_temporary)
write.csv(Haemophilus_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_HINF
...
#################################
df_Haemophilus_kk_cor= as.data.frame(multi_species_df_HINF)
m_BS_stress <- as.data.frame(matrix(0, nrow = 3, ncol = ncol(df_Haemophilus_kk_cor)))

colnames(m_BS_stress)=colnames(df_Haemophilus_kk_cor)

m_BS_stress_1 = rbind(df_Haemophilus_kk_cor[c(1, 2, 3),],m_BS_stress)

m_BS_stress_2 = df_Haemophilus_kk_cor[4:33,]

df_Haemophilus_kk_cor2=rbind(m_BS_stress_1,m_BS_stress_2)

rownames(df_Haemophilus_kk_cor2)=c("As_1","As_2","As_3","Bs_1","Bs_2","Bs_3","Ctrl_1","Ctrl_2","Ctrl_3","Li_1","Li_2","Li_3","Mig_1","Mig_2","Mig_3","Nd_1","Nd_2","Nd_3","Ns_1","Ns_2","Ns_3","Oss_1","Oss_2","Oss_3","Oxs_1","Oxs_2","Oxs_3","Sp_1","Sp_2","Sp_3","Tm_1","Tm_2","Tm_3","Vic_1","Vic_2","Vic_3")

multi_species_df_HINF=df_Haemophilus_kk_cor2

write.csv(multi_species_df_HINF,"multi_species_df_HINF.csv")
#################################
```