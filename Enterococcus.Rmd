...
#Reading the data
Enterococcus = read.csv("~/Desktop/Enterococcus/Enterococcus_faecalis_R_input.csv", sep=";")
dim(Enterococcus) #analising input

#getting TPM values
Enterococcus_expr =as.data.frame(Enterococcus[,2:ncol(Enterococcus)])
dim(Enterococcus_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Enterococcus_expr) = Enterococcus[,1]

#filtering out missing values
gsg = goodSamplesGenes(Enterococcus_expr,verbose = 3)
if (!gsg$allOK)
{
Enterococcus_expr=Enterococcus_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Enterococcus_expr)[2]
```
...
sampleTree = hclust(dist(Enterococcus_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Enterococcus_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
## Note Power=16, R2 over 0.80, module size=15

## Network generation

```{r Networkgeneration}
net = blockwiseModules(Enterococcus_expr, power = 16, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Enterococcus_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Enterococcus_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Enterococcus_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Enterococcus_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Enterococcus_expr), cex.main=2)
...
##############################
multi_species_df_ENTFA_temporary = as.data.frame(ME)
rownames(multi_species_df_ENTFA_temporary) =generic_stress_sample_names
Enterococcus_expr_1 = scale(Enterococcus_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Enterococcus_expr_1)*36)
Module_names <- rep(colnames(Enterococcus_expr_1),36)
lengthOfVector_col=ncol(Enterococcus_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Enterococcus_expr_1[j,i]
Module_names[position_ME] <-      colnames(Enterococcus_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Enterococcus_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_ENTFA_temporary)=module_renamed
multi_species_df_ENTFA=multi_species_df_ENTFA_temporary
write.csv(Enterococcus_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_ENTFA_temporary)=module_renamed
multi_species_df_ENTFA=cbind(multi_species_df_ENTFA,multi_species_df_ENTFA_temporary)
write.csv(Enterococcus_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_ENTFA
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="ENTFA_non_correlated"
colnames(multi_species_df_ENTFA_temporary)=module_renamed
multi_species_df_ENTFA=cbind(multi_species_df_ENTFA,multi_species_df_ENTFA_temporary)
write.csv(Enterococcus_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_ENTFA
...
#################################
write.csv(multi_species_df_ENTFA,"multi_species_df_ENTFA.csv")
#################################
```