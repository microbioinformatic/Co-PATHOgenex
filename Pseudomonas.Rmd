...
#Reading the data
Pseudomonas = read.csv("~/Desktop/Pseudomonas_aeruginosa/Pseudomonas_aeruginosa_R_input.csv", sep=";")
dim(Pseudomonas) #analising input

#getting TPM values
Pseudomonas_expr =as.data.frame(Pseudomonas[,2:ncol(Pseudomonas)])
dim(Pseudomonas_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Pseudomonas_expr) = Pseudomonas[,1]

#filtering out missing values
gsg = goodSamplesGenes(Pseudomonas_expr,verbose = 3)
if (!gsg$allOK)
{
Pseudomonas_expr=Pseudomonas_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Pseudomonas_expr)[2]
```
...
sampleTree = hclust(dist(Pseudomonas_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Pseudomonas_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
## Note Power=9, R2 over 0.90, module size=15

## Network generation

```{r Networkgeneration}
net = blockwiseModules(Pseudomonas_expr, power = 9, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Pseudomonas_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Pseudomonas_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Pseudomonas_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Pseudomonas_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Pseudomonas_expr), cex.main=2)
...
##############################
multi_species_df_PSEUDO_temporary = as.data.frame(ME)
rownames(multi_species_df_PSEUDO_temporary) =generic_stress_sample_names
Pseudomonas_expr_1 = scale(Pseudomonas_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Pseudomonas_expr_1)*36)
Module_names <- rep(colnames(Pseudomonas_expr_1),36)
lengthOfVector_col=ncol(Pseudomonas_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Pseudomonas_expr_1[j,i]
Module_names[position_ME] <-      colnames(Pseudomonas_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Pseudomonas_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_PSEUDO_temporary)=module_renamed
multi_species_df_PSEUDO=multi_species_df_PSEUDO_temporary
write.csv(Pseudomonas_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_PSEUDO_temporary)=module_renamed
multi_species_df_PSEUDO=cbind(multi_species_df_PSEUDO,multi_species_df_PSEUDO_temporary)
write.csv(Pseudomonas_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_PSEUDO
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed=paste(module_renamed,"magenta",sep="")
colnames(multi_species_df_PSEUDO_temporary)=module_renamed
multi_species_df_PSEUDO=cbind(multi_species_df_PSEUDO,multi_species_df_PSEUDO_temporary)
write.csv(Pseudomonas_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_PSEUDO
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="PSEUDO_non_correlated"
colnames(multi_species_df_PSEUDO_temporary)=module_renamed
multi_species_df_PSEUDO=cbind(multi_species_df_PSEUDO,multi_species_df_PSEUDO_temporary)
write.csv(Pseudomonas_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_PSEUDO
...
#################################
write.csv(multi_species_df_PSEUDO,"multi_species_df_PSEUDO.csv")
#################################
```