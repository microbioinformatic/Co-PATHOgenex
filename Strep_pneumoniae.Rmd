...
#Reading the data
Strep_pneumoniae = read.csv("~/Desktop/Streptococcus_pneumoniae/Streptococcus_pneumoniae_R_input.csv", sep=";")
dim(Strep_pneumoniae) #analising input

#getting TPM values
Strep_pneumoniae_expr =as.data.frame(Strep_pneumoniae[,2:ncol(Strep_pneumoniae)])
dim(Strep_pneumoniae_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Strep_pneumoniae_expr) = Strep_pneumoniae[,1]

#filtering out missing values
gsg = goodSamplesGenes(Strep_pneumoniae_expr,verbose = 3)
if (!gsg$allOK)
{
Strep_pneumoniae_expr=Strep_pneumoniae_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Strep_pneumoniae_expr)[2]
```
...
sampleTree = hclust(dist(Strep_pneumoniae_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Strep_pneumoniae_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
## Note Power=14, R2 over 0.80, module size=15

## Network generation

```{r Networkgeneration}
net = blockwiseModules(Strep_pneumoniae_expr, power = 14, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Strep_pneumoniae_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Strep_pneumoniae_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Strep_pneumoniae_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Strep_pneumoniae_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Strep_pneumoniae_expr), cex.main=2)
...
##############################
multi_species_df_STRPN_temporary = as.data.frame(ME)
rownames(multi_species_df_STRPN_temporary) =generic_stress_sample_names
Strep_pneumoniae_expr_1 = scale(Strep_pneumoniae_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Strep_pneumoniae_expr_1)*36)
Module_names <- rep(colnames(Strep_pneumoniae_expr_1),36)
lengthOfVector_col=ncol(Strep_pneumoniae_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Strep_pneumoniae_expr_1[j,i]
Module_names[position_ME] <-      colnames(Strep_pneumoniae_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Strep_pneumoniae_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_STRPN_temporary)=module_renamed
multi_species_df_STRPN=multi_species_df_STRPN_temporary
write.csv(Strep_pneumoniae_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_STRPN_temporary)=module_renamed
multi_species_df_STRPN=cbind(multi_species_df_STRPN,multi_species_df_STRPN_temporary)
write.csv(Strep_pneumoniae_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_STRPN
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="STRPN_non_correlated"
colnames(multi_species_df_STRPN_temporary)=module_renamed
multi_species_df_STRPN=cbind(multi_species_df_STRPN,multi_species_df_STRPN_temporary)
write.csv(Strep_pneumoniae_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_STRPN
...
#################################
write.csv(multi_species_df_STRPN,"multi_species_df_STRPN.csv")
#################################
```