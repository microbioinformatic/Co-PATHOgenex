...
#Reading the data
Aggregatibacter = read.csv("~/Desktop/Aggregatibacter_actinomycetemcomitans/Aggregatibacter_actinomycetemcomitans_R_input.csv", sep=";")
dim(Aggregatibacter) #analising input

#getting TPM values
Aggregatibacter_expr =as.data.frame(Aggregatibacter[,2:ncol(Aggregatibacter)])
dim(Aggregatibacter_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Aggregatibacter_expr) = Aggregatibacter[,1]

#filtering out missing values
gsg = goodSamplesGenes(Aggregatibacter_expr,verbose = 3)
if (!gsg$allOK)
{
Aggregatibacter_expr=Aggregatibacter_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Aggregatibacter_expr)[2]
```
...
sampleTree = hclust(dist(Aggregatibacter_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Aggregatibacter_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Aggregatibacter_expr, power = 10, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Aggregatibacter_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Aggregatibacter_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Aggregatibacter_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Aggregatibacter_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Aggregatibacter_expr), cex.main=2)
...
##############################
multi_species_df_AGGA_temporary = as.data.frame(ME)
rownames(multi_species_df_AGGA_temporary) =generic_stress_sample_names
Aggregatibacter_expr_1 = scale(Aggregatibacter_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Aggregatibacter_expr_1)*36)
Module_names <- rep(colnames(Aggregatibacter_expr_1),36)
lengthOfVector_col=ncol(Aggregatibacter_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Aggregatibacter_expr_1[j,i]
Module_names[position_ME] <-      colnames(Aggregatibacter_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Aggregatibacter_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_AGGA_temporary)=module_renamed
multi_species_df_AGGA=multi_species_df_AGGA_temporary
write.csv(Aggregatibacter_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_AGGA_temporary)=module_renamed
multi_species_df_AGGA=cbind(multi_species_df_AGGA,multi_species_df_AGGA_temporary)
write.csv(Aggregatibacter_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_AGGA
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="AGGA_non_correlated"
colnames(multi_species_df_AGGA_temporary)=module_renamed
multi_species_df_AGGA=cbind(multi_species_df_AGGA,multi_species_df_AGGA_temporary)
write.csv(Aggregatibacter_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_AGGA
...
#################################
write.csv(multi_species_df_AGGA,"multi_species_df_AGGA.csv")
#################################
```