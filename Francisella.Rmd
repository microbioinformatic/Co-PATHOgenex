...
#Reading the data
Francisella = read.csv("~/Desktop/Francisella/Francisella_tularensis_R_input.csv", sep=";")
dim(Francisella) #analising input

#getting TPM values
Francisella_expr =as.data.frame(Francisella[,2:ncol(Francisella)])
dim(Francisella_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Francisella_expr) = Francisella[,1]

#filtering out missing values
gsg = goodSamplesGenes(Francisella_expr,verbose = 3)
if (!gsg$allOK)
{
Francisella_expr=Francisella_expr[gsg$goodSamples,gsg$goodGenes]
}

#Number of genes after filtering
dim(Francisella_expr)[2]
```
...
sampleTree = hclust(dist(Francisella_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Francisella_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Francisella_expr, power = 12, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Francisella_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Francisella_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Francisella_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Francisella_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Francisella_expr), cex.main=2)
...
##############################
multi_species_df_FRAT_temporary = as.data.frame(ME)
rownames(multi_species_df_FRAT_temporary) =generic_stress_sample_names
Francisella_expr_1 = scale(Francisella_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Francisella_expr_1)*36)
Module_names <- rep(colnames(Francisella_expr_1),36)
lengthOfVector_col=ncol(Francisella_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Francisella_expr_1[j,i]
Module_names[position_ME] <-      colnames(Francisella_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Francisella_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_FRAT_temporary)=module_renamed
multi_species_df_FRAT=multi_species_df_FRAT_temporary
write.csv(Francisella_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_FRAT_temporary)=module_renamed
multi_species_df_FRAT=cbind(multi_species_df_FRAT,multi_species_df_FRAT_temporary)
write.csv(Francisella_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_FRAT
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="FRAT_non_correlated"
colnames(multi_species_df_FRAT_temporary)=module_renamed
multi_species_df_FRAT=cbind(multi_species_df_FRAT,multi_species_df_FRAT_temporary)
write.csv(Francisella_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_FRAT
...
#################################
write.csv(multi_species_df_FRAT,"multi_species_df_FRAT.csv")
#################################
```