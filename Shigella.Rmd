...
#Reading the data
Shigella = read.csv("~/Desktop/Shigella_flexneri/Shigella_flexneri_R_input.csv", sep=";")
dim(Shigella) #analising input

#getting TPM values
Shigella_expr =as.data.frame(Shigella[,2:ncol(Shigella)])
dim(Shigella_expr) #analysing dataset

#transforming dataset to start running WGCNA functions
rownames(Shigella_expr) = Shigella[,1]

#filtering out missing values
gsg = goodSamplesGenes(Shigella_expr,verbose = 3)
if (!gsg$allOK)
{
Shigella_expr=Shigella_expr[gsg$goodSamples,gsg$goodGenes]
}
...
sampleTree = hclust(dist(Shigella_expr), method="average")
par(mar = c(0,4,2,0))
...
sft = pickSoftThreshold(Shigella_expr, powerVector = powers, verbose = 5)
par(mfrow = c(1,2));
cex1 = 0.6;
...
```{r Networkgeneration}
net = blockwiseModules(Shigella_expr, power = 12, TOMType = "unsigned", minModuleSize = 15, reassignThreshold = 0, mergeCutHeight = 0.25, numericLabels = TRUE, pamRespectsDendro = FALSE, saveTOMs = TRUE, saveTOMFileBase = "Shigella_expr", verbose = 3)
```
...
save(MEs, moduleLabels, moduleColors, geneTree,file ="Shigella_expr.RData")
```

##  Dendrograms (clustering trees) of the module eigengenes
```{r DendogramME, echo=TRUE}
datME=moduleEigengenes(Shigella_expr,moduleColors)$eigengenes
...
plotMat(t(scale(Shigella_expr[,colorh1==which.module ]) ),nrgcols=30,rlabels=F,rcols=which.module,clabels=rownames(Shigella_expr), cex.main=2)
...
##############################
multi_species_df_SHIF_temporary = as.data.frame(ME)
rownames(multi_species_df_SHIF_temporary) =generic_stress_sample_names
Shigella_expr_1 = scale(Shigella_expr[,colorh1==which.module ])
ME_lengthOfVector=rep(NA, ncol(Shigella_expr_1)*36)
Module_names <- rep(colnames(Shigella_expr_1),36)
lengthOfVector_col=ncol(Shigella_expr_1)
position_ME=1
for(i in 1:lengthOfVector_col) {
for(j in 1:36) {
ME_lengthOfVector[position_ME] <- Shigella_expr_1[j,i]
Module_names[position_ME] <-      colnames(Shigella_expr_1)[i]
position_ME = position_ME+1
}
position_ME = i*36+1
}
stress=rep(generic_stress_sample_names,ncol(Shigella_expr_1))
...
#####################################################
module_renamed
colnames(multi_species_df_SHIF_temporary)=module_renamed
multi_species_df_SHIF=multi_species_df_SHIF_temporary
write.csv(Shigella_expr_1,paste(module_renamed,".csv",sep=""))
####################################################
#####################################################
which.module="pink"
ME=datME[, paste("ME",which.module, sep="")]
...
module_renamed
colnames(multi_species_df_SHIF_temporary)=module_renamed
multi_species_df_SHIF=cbind(multi_species_df_SHIF,multi_species_df_SHIF_temporary)
write.csv(Shigella_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_SHIF
#####################################
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed="SHIF_non_correlated"
colnames(multi_species_df_SHIF_temporary)=module_renamed
multi_species_df_SHIF=cbind(multi_species_df_SHIF,multi_species_df_SHIF_temporary)
write.csv(Shigella_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_SHIF
...
if (length(positive_eigengene_name) > 0 && length(negative_eigengene_name)> 0) {
  ...
  module_renamed=paste(module_renamed,"_UP",sep="")
   ...
   module_renamed=paste(module_renamed,which.module,sep="")}
#####################################################
module_renamed=paste(module_renamed,"lightyellow",sep="")
colnames(multi_species_df_SHIF_temporary)=module_renamed
multi_species_df_SHIF=cbind(multi_species_df_SHIF,multi_species_df_SHIF_temporary)
write.csv(Shigella_expr_1,paste(module_renamed,".csv",sep=""))
multi_species_df_SHIF
...
#################################
write.csv(multi_species_df_SHIF,"multi_species_df_SHIF.csv")
#################################
```